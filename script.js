/***** CONFIG *****/
// 새로 받은 exec 주소 (필요시, 이 주소를 한번 열어 리다이렉트되는
// googleusercontent 주소 전체로 교체하면 더 안정적입니다)
const BACKEND_URL =
...
const withQ = (params) => `${BACKEND_URL}${BACKEND_URL.includes('?') ? '&' : '?'}${qs(params)}`;
...
const stars = (n)=>{const r=Math.max(1,Math.min(5,n|0));return '★'.repeat(r)+'☆'.repeat(5-r);};

/***** API (CORS 프리: GET) *****/
async function apiList(){
  const res = await fetch(withQ({ action:'list', t: Date.now() }), { method:'GET', cache:'no-store' });
  return res.json();
}
async function apiAdd({ name, rating, content }){
  ...
  try { return await res.json(); } catch { return { ok:true, via:'GET' }; }
}
